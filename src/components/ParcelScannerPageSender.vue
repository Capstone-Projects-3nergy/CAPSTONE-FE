...
async function extractParcelInfo(imageDataUrl) {
  ...
  try {
    ...
    // 👤 Recipient Search with more keywords
    ...
    for (const kw of recipientKeywords) {
      ...
      if (match) {
        ...
        name = name.split(/[\d]/)[0].trim() // Stop at phone number
        if (name.length >= 2) {
    ...
    // 📤 Sender Search with more keywords
    ...
    // Define patterns and link them to potential companies
    ...
    for (const pattern of trackingPatterns) {
      ...
      if (match) {
        ...
        // Try to identify company based on pattern or keywords
    ...
    // Search for Company names specifically if not found by tracking pattern
    if (!info.companyId) {
